...
def select_blend_option(page, option_text):    
...
def bot_command(page, command):
    try:
        ...
        prompt_option.click()
        print("Prompt option selected.")
        time.sleep(random.randint(1, 5))
        # Call the select_images_to_blend function.
        select_images_to_blend(page)
    except Exception as e:
        print("An error occurred while entering the bot command:", e)

# Function to select images to blend.
def select_images_to_blend(page):
    image1_drop_box = page.locator("li[role=\"listitem\"]:has-text(\"image1Drag and drop or click to upload file\")").get_by_role("button", name="Drag and drop or click to upload file")
    # Select image1 dropbox.
    image1_drop_box.click()
    time.sleep(random.randint(1, 5))
    # Select the first image from the File Explorer.
    # CODE TO SELECT FIRST IMAGE FROM FILE EXPLORER GOES HERE
    # image2_drop_box = page.locator("li[role=\"listitem\"]:has-text(\"image2Drag and drop or click to upload file\")").get_by_role("button", name="Drag and drop or click to upload file")
    # # Select image2 dropbox.
    # image2_drop_box.click()
    # time.sleep(random.randint(1, 5))
    # # Select the second image from the File Explorer.
    # # CODE TO SELECT SECOND IMAGE FROM FILE EXPLORER GOES HERE
    # # Press the Enter key.
    # page.keyboard.press("Enter")
